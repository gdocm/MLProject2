...
The :mod:`gplearn_MLAA.functions` module contains all of the functions used by
gplearn_MLAA programs. It also contains helper methods for a user to define their
...
class _Function(object):
    ...
    def __call__(self, *args):
        ...
        try:
            if len(results) < 129:
                print(results)
                raise Exception("Length of intermediate_result is not 129")
        except:
            print(results)
            raise Exception("Length of intermediate_result is not 129")
        return results
